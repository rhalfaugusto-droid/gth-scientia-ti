...
def parse_nfe_xml(xml_content: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # 2. Definição dos namespaces (necessário para NFe)
        # O namespace principal da NFe é geralmente 'http://www.portalfiscal.inf.br/nfe'
        ns = {'nfe': 'http://www.portalfiscal.inf.br/nfe'}
        
        # 3. Extração de Dados da NFe (Identificação)
        ...
        if ide_node is None or emit_node is None or dest_node is None:
            return None # XML não parece ser uma NFe válida
            
        # 4. Extração de Dados da NFe (Valores Totais)
        ...
        # 5. Extração de Dados dos Itens (Produtos)
        items = []
        for det_node in root.findall('.//nfe:det', namespaces=ns):
            ...
            item_data = {
                ...
                'qCom': float(prod_node.find('nfe:qCom', namespaces=ns).text) if prod_node is not None and prod_node.find('nfe:qCom', namespaces=ns) is not None else 0.0,
                'vUnCom': float(prod_node.find('nfe:vUnCom', namespaces=ns).text) if prod_node is not None and prod_node.find('nfe:vUnCom', namespaces=ns) is not None else 0.0,
                'vProd': float(prod_node.find('nfe:vProd', namespaces=ns).text) if prod_node is not None and prod_node.find('nfe:vProd', namespaces=ns) is not None else 0.0,
                'NCM': prod_node.find('nfe:NCM', namespaces=ns).text if prod_node is not None else None,
                'CST_ICMS': imposto_node.find('.//nfe:ICMS/*/nfe:CST', namespaces=ns).text if imposto_node is not None else None,
                'vICMS': float(imposto_node.find('.//nfe:ICMS/*/nfe:vICMS', namespaces=ns).text) if imposto_node is not None and imposto_node.find('.//nfe:ICMS/*/nfe:vICMS', namespaces=ns) is not None else 0.0,
                # ... outros campos relevantes para a transição (ex: CEST, CFOP)
            }
            items.append(item_data)

        # 6. Consolidação dos Dados
        extracted_data = {
            'nfe_id': root.find('.//nfe:infNFe', namespaces=ns).get('Id'),
            'chave_acesso': root.find('.//nfe:infNFe', namespaces=ns).get('Id')[3:],
            'data_emissao': ide_node.find('nfe:dhEmi', namespaces=ns).text if ide_node.find('nfe:dhEmi', namespaces=ns) is not None else ide_node.find('nfe:dEmi', namespaces=ns).text,
            'emitente': {
            ...
            'destinatario': {
                'CNPJ': dest_node.find('nfe:CNPJ', namespaces=ns).text if dest_node.find('nfe:CNPJ', namespaces=ns) is not None else dest_node.find('nfe:CPF', namespaces=ns).text,
